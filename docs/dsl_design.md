...
Data Availability: The system assumes the input DataFrame contains standard OHLCV columns (open, high, low, close, volume).

Case Insensitivity: The NLP engine normalizes input to lowercase, but the DSL parser expects keywords (AND, OR, ENTRY) to be standardized (handled by the pipeline).

Validation: The execution engine validates that all referenced variables exist in the dataset before execution to prevent runtime crashes.